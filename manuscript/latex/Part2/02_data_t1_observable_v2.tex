...
\right\|^2 .
\end{equation}
Here, $\hat{\mathbf{J}}_i(t)$ is an affine map of finite-lag relative
displacements, not a constitutive deformation tensor or an inferred
force-gradient. Because the fit uses focal-centered relative coordinates,
local translation is already removed. The finite-lag non-affine residual
velocity is
\begin{equation}
\mathbf{u}^{NA}_{ij}(t)=
\frac{
...
to focal individual $i$. Focal samples with non-finite or numerically zero
swarm-centered radius were excluded before this projection. The tangential
residual is
\begin{equation}
